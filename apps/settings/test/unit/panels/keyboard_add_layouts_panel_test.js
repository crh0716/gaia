...
suite('KeyboardAddLayoutsPanel', function() {
  var map = {
    ...
    'panels/keyboard_add_layouts_panel/panel': {
      'shared/keyboard_helper': 'shared_mocks/mock_keyboard_helper',
  ...
  suite('KeyboardAddLayoutsPanel', function() {
    ...
    test('Save to settings when leaving panel', function() {
      sinon.spy(MockKeyboardHelper, 'saveToSettings');

      var panel = this.KeyboardAddLayoutsPanel();
      panel.init(this.root);
      panel.beforeHide();

      sinon.assert.called(MockKeyboardHelper.saveToSettings);
      MockKeyboardHelper.saveToSettings.restore();
    });
  });
});
